    ...
    getScriptSrc = () => {
      ...
      if (true) {
        if (isEsmImportMetaUrlHardcodedAsFileUri) {
          const URL2 = URL;
          return new URL(new URL2('ort.jspi.mjs', import.meta.url).href, origin)
            .href;
        }
    ...
    importWasmModule = async (
      ...
      if (useEmbeddedModule) {
        if (!scriptSrc) {
          ...
          useEmbeddedModule =
            isSameOrigin(scriptSrc) || (isWasmOverridden && !isMultiThreaded);
        }
      }
      if (useEmbeddedModule) {
          ...
          case 'webgpu':
            if (true) {
              ...
              if (typeof ep !== 'string') {
                ...
                if (
                  typeof enableGraphCapture === 'boolean' &&
                  enableGraphCapture
                ) {
    ...
    TensorIdTracker = class {
      ...
      async ensureTensor(sessionId, dataType, shape, copyOld) {
        ...
        if (!opLimits?.input.dataTypes.includes(dataType)) {
          ...
          if (
            !fallbackDataType ||
            opLimits?.input.dataTypes.includes(fallbackDataType)
          ) {
            throw new Error(
              `WebNN backend does not support data type: ${dataType}`
            );
    ...
    WebNNBackend = class {
      ...
      registerMLContext(sessionId, mlContext) {
        ...
        if (!this.mlOpSupportLimitsBySessionId.has(sessionId)) {
          this.mlOpSupportLimitsBySessionId.set(
            sessionId,
