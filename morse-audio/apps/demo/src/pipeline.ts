...
export interface DemoSettings {
  ...
  ionospheric: { enabled: boolean; depth: number; rate: number; components: number };
  multipath: { enabled: boolean; paths: number; maxDelayMs: number; decay: number; phaseSpread: number };
  doppler: { enabled: boolean; spreadHz: number; components: number };
  pitchWobble: { enabled: boolean; amplitude: number; rate: number; phase: number };
  ...
  cwQrm: { enabled: boolean; count: number; separationHz: number; powerDb: number; wpm: number; text: string };
  broadband: { enabled: boolean; centerFrequency: number; bandwidth: number; powerDb: number };
  agc: { enabled: boolean; attackMs: number; releaseMs: number; targetLevel: number; maxGain: number };
  bandpass: { enabled: boolean; bandwidth: number; centerFrequency: number; lockToTone: boolean; stages: number };
}
...
function multipathOptions(s: DemoSettings): MorseAudioConfig['multipath'] {
  ...
  const paths = Array.from({ length: s.multipath.paths }, (_, i) => {
    ...
    return {
      ...
      amplitude: Math.pow(s.multipath.decay, n),
...
function settingsToConfig(s: DemoSettings): MorseAudioConfig {
  return {
    ...
    pitchWobble: s.pitchWobble.enabled
      ? { amplitude: s.pitchWobble.amplitude, rate: s.pitchWobble.rate, phase: s.pitchWobble.phase }
      : undefined,
    rayleigh: s.rayleigh.enabled
      ? { bandwidth: s.rayleigh.bandwidth, depth: s.rayleigh.depth }
      : undefined,
    flutter: s.flutter.enabled ? { rate: s.flutter.rate, depth: s.flutter.depth } : undefined,
    ...
    cwQrm: s.cwQrm.enabled
      ? Array.from({ length: s.cwQrm.count }, (_, i) => ({
          frequencySeparation: s.cwQrm.separationHz * (i % 2 === 0 ? 1 : -1) * (1 + Math.floor(i / 2)),
...
export function generateDemoAudio(s: DemoSettings): GenerateResult {
  ...
  return {
    dataUri: getDataURI(getWavData(samples, result.metadata.config.sampleRate), getMIMEType()),
...
export const DEFAULT_SETTINGS: DemoSettings = {
  ...
  multipath: { enabled: false, paths: 3, maxDelayMs: 8, decay: 0.55, phaseSpread: 1 },
  ...
  cwQrm: { enabled: false, count: 1, separationHz: 280, powerDb: -6, wpm: 24, text: '' },
  broadband: { enabled: false, centerFrequency: 1700, bandwidth: 900, powerDb: -8 },
  agc: { enabled: false, attackMs: 10, releaseMs: 100, targetLevel: 0.7, maxGain: 10 },
  bandpass: { enabled: false, bandwidth: 500, centerFrequency: 600, lockToTone: true, stages: 4 },
};
