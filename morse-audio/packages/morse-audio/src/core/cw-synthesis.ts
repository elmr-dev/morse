...
import type { ChirpOptions, RayleighFadingOptions, FlutterOptions, BuzzOptions } from '../pileup/types';
...
export function synthesizeCw(options: CwSynthesisOptions): CwSynthesisResult {
  ...
  const contentOffsetSamples = Math.max(0, Math.ceil((startDelayMs / 1000) * sampleRate));
  const minTotalSamples = contentOffsetSamples + contentEnvelope.length;
  const totalSamples = totalDurationMs === undefined
    ? minTotalSamples
    : Math.max(minTotalSamples, Math.ceil((totalDurationMs / 1000) * sampleRate));
  ...
  if (pitchWobble) {
    const wobbleOffsets = generatePitchWobbleOffsets(envelope.length, pitchWobble, sampleRate);
    if (freqOffsets) {
      for (let i = 0; i < freqOffsets.length; i++) freqOffsets[i] += wobbleOffsets[i];
    } else {
      freqOffsets = wobbleOffsets;
    }
  }

  const rayleighProcessor = rayleigh ? new RayleighFading(rayleigh, sampleRate, seed + 1) : null;
  const flutterProcessor = flutter ? new Flutter(flutter, sampleRate, seed + 2) : null;
