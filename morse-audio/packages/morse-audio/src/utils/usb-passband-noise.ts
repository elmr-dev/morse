...
function makeBandpassKernel(lowCutHz: number, highCutHz: number, sampleRate: number, taps: number): Float32Array {
...
export function applyUsbPassbandFilter(
  ...
  sampleRate: number,
  options: Omit<UsbPassbandNoiseOptions, 'length' | 'sampleRate' | 'seed' | 'targetRms' | 'agcDepth' | 'agcRateHz'> = {}
): Float32Array {
  ...
  const stopbandLeakageDb = options.stopbandLeakageDb ?? DEFAULT_USB_STOPBAND_LEAKAGE_DB;
  const leakageGain = Math.pow(10, stopbandLeakageDb / 20);
  const kernel = makeBandpassKernel(lowCutHz, highCutHz, sampleRate, filterTaps);
 ...
 */
export function generateUsbPassbandNoise(options: UsbPassbandNoiseOptions): Float32Array {
  const {
    length,
    sampleRate,
    seed = 0x755342, // 'USB'
  ...
  if (pops && popRateHz > 0 && popAmplitude > 0) {
    ...
    while (popAt < length) {
      ...
      const decayMs = popMinDecayMs + prng() * Math.max(0, popMaxDecayMs - popMinDecayMs);
      const decaySamples = Math.max(1, (decayMs / 1000) * sampleRate);
      const burstSamples = Math.min(length - popAt, Math.ceil(decaySamples * 4));
  ...
  if (agcDepth > 0 && agcRateHz > 0) {
    ...
    for (let i = 0; i < output.length; i++) {
      ...
      const wander = 0.65 * Math.sin(2 * Math.PI * agcRateHz * t + phase1)
