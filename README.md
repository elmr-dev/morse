...
`bunx turbo build` does **not** produce the model — `cw-model` has no `build` task.
...
1. `bunx turbo check:fix` on staged `*.{ts,tsx}` (auto-fixes are re-staged into the commit)
2. `bunx turbo typecheck`
3. `bunx turbo test`

A `pre-push` gate then runs `bunx turbo build` — it catches bundler failures (tsup
